...
import { createDelivery, deleteDelivery, subscribeToDeliveries } from "../src/services/deliveryService";
...
type Delivery = { id:string; order:string; customer:string; phone:string; address:string; district:string; amount:number; deliveryFee:number; payment:string; priority:"Normal"|"Alta"|"Urgente"; status:Status; driver:string; time:string; latitude:number; longitude:number };
...
function loadStore():StoreCfg{try{const raw=typeof localStorage!=="undefined"?localStorage.getItem(STORE_KEY):null;const s=raw?JSON.parse(raw):null;if(s&&typeof s.latitude==="number")return s;}catch{} return {name:"",phone:"",address:"",latitude:-13.0033,longitude:-38.4581};}
...
function fromRecord(item:DeliveryRecord):Delivery{return {id:item.id,order:item.orderNumber,customer:item.customerName,phone:item.phone,address:[item.address,item.number].filter(Boolean).join(", "),district:item.district,amount:item.amount,deliveryFee:item.deliveryFee||0,payment:item.paymentMethod,priority:item.priority==="urgent"?"Urgente":item.priority==="high"?"Alta":"Normal",status:recordStatus[item.status],driver:"Não atribuído",time:"14:00",latitude:item.latitude||STORE_POINT.latitude,longitude:item.longitude||STORE_POINT.longitude}}
...
function DeliveryApp({profile,demoMode}:{profile:User;demoMode:boolean}) {
  ...
  async function saveDelivery(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const address=String(f.get("address"));const local:Delivery={id:String(Date.now()),order:String(f.get("order")||"#1853"),customer:String(f.get("customer")),phone:String(f.get("phone")),address,district:String(f.get("district")),amount:Number(f.get("amount")),deliveryFee:Number(f.get("deliveryFee")),payment:String(f.get("payment")),priority:"Normal",status:"Aguardando",driver:"Não atribuído",time:"14:00",latitude:Number(f.get("latitude"))||STORE_POINT.latitude,longitude:Number(f.get("longitude"))||STORE_POINT.longitude};try{if(demoMode)setDeliveries(v=>[local,...v]);else await createDelivery({companyId:profile.companyId,orderNumber:local.order,customerName:local.customer,phone:local.phone,address,number:String(f.get("number")||"s/n"),district:local.district,city:String(f.get("city")||""),postalCode:String(f.get("postalCode")||""),latitude:local.latitude,longitude:local.longitude,amount:local.amount,deliveryFee:local.deliveryFee,paymentMethod:local.payment,notes:String(f.get("notes")||""),priority:"normal",source:ocrData?"ocr":"manual",status:"pending"});setModal(null);setOcrData(null);notify("Entrega cadastrada com taxa de "+money(local.deliveryFee));}catch(error){notify(error instanceof Error?error.message:"Não foi possível salvar a entrega.")}}
...
function NewDelivery({close,save,prefill}:{close:()=>void;save:(e:React.FormEvent<HTMLFormElement>)=>void;prefill?:Record<string,string>|null}){const pf=(k:string)=>prefill?.[k]||"";
  ...
  return <div className="overlay"><div className="modal large"><div className="modal-head"><div><h2>Nova entrega</h2><p>Cadastre o pedido e localize gratuitamente no mapa.</p></div><button onClick={close}><X/></button></div><form ref={form} onSubmit={save}><input type="hidden" name="latitude" value={location?.latitude||""}/><input type="hidden" name="longitude" value={location?.longitude||""}/><div className="form-grid"><label>Número do pedido<input name="order" defaultValue={pf("order")||"#1853"} required/></label><label>Nome do cliente<input name="customer" defaultValue={pf("customer")} placeholder="Nome completo" required/></label><label>Telefone<input name="phone" defaultValue={pf("phone")} placeholder="(71) 99999-9999" required/></label><label>CEP<input name="postalCode" defaultValue={pf("postalCode")} placeholder="00000-000"/></label><label className="full">Rua e número<input name="address" defaultValue={pf("address")} placeholder="Ex.: Rua das Acácias, 120" required/></label><label>Bairro<input name="district" defaultValue={pf("district")} placeholder="Bairro" required/></label><label>Cidade<input name="city" defaultValue={pf("city")||"Teixeira de Freitas"}/></label><label>Valor do pedido<input name="amount" type="number" step="0.01" min="0" defaultValue={pf("amount")} placeholder="R$ 0,00" required/></label><label>Valor da entrega<input name="deliveryFee" type="number" step="0.01" min="0" defaultValue={pf("deliveryFee")} placeholder="R$ 0,00" required/><small className="field-help">Taxa que será somada ao faturamento das entregas</small></label><label>Forma de pagamento<select name="payment" defaultValue={pf("payment")||"Pix"}><option>Pix</option><option>Dinheiro</option><option>Cartão</option><option>Pago</option></select></label><label className="full">Observações<textarea name="notes" placeholder="Ponto de referência, instruções ou troco"/></label></div><div className="locate"><MapPin/><div><b>Localização do endereço</b><span>{locationText}</span></div><div className="locate-actions"><button type="button" disabled={locating} onClick={locateAddress}>Buscar endereço</button><button type="button" disabled={locating} onClick={useGps}><Navigation size={14}/> Usar GPS</button></div></div><div className="modal-actions"><button type="button" onClick={close}>Cancelar</button><button className="primary" type="submit">Confirmar e cadastrar entrega</button></div></form></div></div>
}
...
function OCR({close,done}:{close:()=>void;done:(fields:Record<string,string>)=>void}){
  ...
  async function process(){if(!file)return;setReading(true);setProgress(1);try{const {readReceipt}=await import("../src/services/ocrService");const res=await readReceipt(file,(n)=>setProgress(Math.max(1,n)));done(parseComanda(res.rawText));}catch(err){console.error(err);alert("Nao consegui ler a imagem. Tente uma foto mais nitida, reta e bem iluminada.");}finally{setReading(false);}}
  return <div className="overlay"><div className="modal ocr-modal"><div className="modal-head"><div><h2>Ler comanda</h2><p>A imagem e lida somente neste aparelho (OCR gratuito).</p></div><button onClick={close}><X/></button></div><input ref={inputRef} type="file" accept="image/*" capture="environment" hidden onChange={pick}/>{!preview?<div className="dropzone"><div><ScanLine/></div><b>Fotografe ou escolha uma comanda</b><span>Use uma foto nitida, bem iluminada e sem inclinacao.</span><button className="primary" onClick={()=>inputRef.current?.click()}><Camera/> Tirar foto</button><button onClick={()=>inputRef.current?.click()}><Upload/> Galeria</button></div>:<><div className="receipt-preview"><img src={preview} alt="comanda" style={{maxHeight:"280px",maxWidth:"100%",objectFit:"contain"}}/><button title="Trocar foto" onClick={()=>inputRef.current?.click()}><RotateCw/></button></div>{(reading||progress>0)?<div className="progress"><div><span style={{width:progress+"%"}}/></div><b>Lendo comanda... {progress}%</b><small>Reconhecendo texto em portugues</small></div>:<button className="primary wide" onClick={process}><ScanLine/> Iniciar leitura</button>}</>}</div></div>;
}
