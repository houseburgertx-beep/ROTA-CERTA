...
function OCR({close,done}:{close:()=>void;done:(fields:Record<string,string>)=>void}){
  ...
  return <div className="overlay"><div className="modal ocr-modal"><div className="modal-head"><div><h2>Ler comanda com IA</h2><p>Firebase AI identifica os dados; OCR local assume se necessário.</p></div><button onClick={close}><X/></button></div><input ref={inputRef} type="file" accept="image/*" capture="environment" hidden onChange={pick}/>{!preview?<div className="dropzone"><div><ScanLine/></div><b>Fotografe ou escolha uma comanda</b><span>Use uma foto nítida, bem iluminada e sem inclinação.</span><button className="primary" onClick={()=>inputRef.current?.click()}><Camera/> Tirar foto</button><button onClick={()=>inputRef.current?.click()}><Upload/> Galeria</button></div>:<><div className="receipt-preview"><img src={preview} alt="comanda" style={{maxHeight:"280px",maxWidth:"100%",objectFit:"contain"}}/><button title="Trocar foto" onClick={()=>inputRef.current?.click()}><RotateCw/></button></div>{(reading||progress>0)?<div className="progress"><div><span style={{width:progress+"%"}}/></div><b>Analisando com IA... {progress}%</b><small>Extraindo somente dados visíveis para sua confirmação</small></div>:<button className="primary wide" onClick={process}><ScanLine/> Analisar com IA</button>}</>}</div></div>;
}
